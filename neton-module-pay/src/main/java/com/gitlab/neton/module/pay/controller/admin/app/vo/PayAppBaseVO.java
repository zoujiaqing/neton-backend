package com.gitlab.neton.module.pay.controller.admin.app.vo;
...
public class PayAppBaseVO {
    ...
    @Schema(description = "应用标识", requiredMode = Schema.RequiredMode.REQUIRED, example = "neton")
    ...
    @Schema(description = "支付结果的回调地址", requiredMode = Schema.RequiredMode.REQUIRED, example = "http://127.0.0.1:8080/pay-callback")
    @NotNull(message = "支付结果的回调地址不能为空")
    @URL(message = "支付结果的回调地址必须为 URL 格式")
    private String orderNotifyUrl;

    @Schema(description = "退款结果的回调地址", requiredMode = Schema.RequiredMode.REQUIRED, example = "http://127.0.0.1:8080/refund-callback")
    @NotNull(message = "退款结果的回调地址不能为空")
    @URL(message = "退款结果的回调地址必须为 URL 格式")
    private String refundNotifyUrl;

    @Schema(description = "转账结果的回调地址", example = "http://127.0.0.1:8080/transfer-callback")
